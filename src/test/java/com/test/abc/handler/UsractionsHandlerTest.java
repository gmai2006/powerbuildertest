...
import com.test.abc.dao.DefaultUsractionsDao;
...
public class UsractionsHandlerTest {
  static final String inputFile = "Usractions.json";
  static UsractionsHandler handler;
  private JpaDao jpa;
  static Gson gson = new GsonBuilder().setDateFormat("yyyy-MM-dd HH:mm:ss.S").create();
  private Usractions[] records;
  ...
  private File createRecordInputStreamFromJsonFile(String inputFile, Charset charset) {
    try {
      ...
      String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
      final Usractions[] records = gson.fromJson(json, Usractions[].class);
      String header =
          createHeader(
  ...
  private String createCsvRecord(final Usractions record) {
    return TestUtils.getObject(record.getId())
        ...
        + ","
        + TestUtils.getObject(record.getDescaction());
  }
