...
import com.test.abc.dao.DefaultMisthZpthesiDao;
...
public class MisthZpthesiHandlerTest {
  ...
  public void testSelect() throws IOException {
    ...
    MisthZpthesi testResult = jpa.find(MisthZpthesi.class, records[0].getKodthesi());
    assertEquals(
  ...
  private File createRecordInputStreamFromJsonFile(String inputFile, Charset charset) {
    try {
      ...
      String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
      final MisthZpthesi[] records = gson.fromJson(json, MisthZpthesi[].class);
      String header =
          createHeader(
  ...
  private String createCsvRecord(final MisthZpthesi record) {
    return TestUtils.getObject(record.getKodthesi())
        + ","
        + TestUtils.getObject(record.getKodxrisi())
        + ","
        + TestUtils.getObject(record.getDescthesi());
  }
