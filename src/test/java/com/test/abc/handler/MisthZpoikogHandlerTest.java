...
import com.test.abc.dao.DefaultMisthZpoikogDao;
...
public class MisthZpoikogHandlerTest {
  ...
  public void testSelect() throws IOException {
    ...
    MisthZpoikog testResult = jpa.find(MisthZpoikog.class, records[0].getKodoikog());
    assertEquals(
  ...
  private File createRecordInputStreamFromJsonFile(String inputFile, Charset charset) {
    try {
      ...
      String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
      final MisthZpoikog[] records = gson.fromJson(json, MisthZpoikog[].class);
      String header =
          createHeader(
  ...
  private String createCsvRecord(final MisthZpoikog record) {
    return TestUtils.getObject(record.getKodoikog())
        + ","
        + TestUtils.getObject(record.getKodxrisi())
        + ","
        + TestUtils.getObject(record.getDescoikog());
  }
