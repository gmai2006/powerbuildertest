...
import com.test.abc.dao.DefaultMisthZpidikotDao;
...
public class MisthZpidikotHandlerTest {
  static final String inputFile = "MisthZpidikot.json";
  static MisthZpidikotHandler handler;
  private JpaDao jpa;
  static Gson gson = new GsonBuilder().setDateFormat("yyyy-MM-dd HH:mm:ss.S").create();
  private MisthZpidikot[] records;
  ...
  public void testSelect() throws IOException {
    ...
    records = gson.fromJson(json, MisthZpidikot[].class);
    assertEquals("match count", count, records.length);
    MisthZpidikot testResult = jpa.find(MisthZpidikot.class, records[0].getKodidikot());
    assertEquals(
  ...
  private File createRecordInputStreamFromJsonFile(String inputFile, Charset charset) {
    try {
      ...
      String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
      final MisthZpidikot[] records = gson.fromJson(json, MisthZpidikot[].class);
      String header =
          createHeader(
  ...
  private String createCsvRecord(final MisthZpidikot record) {
    return TestUtils.getObject(record.getKodidikot())
        + ","
        + TestUtils.getObject(record.getKodxrisi())
        + ","
        + TestUtils.getObject(record.getDescidikot());
  }
