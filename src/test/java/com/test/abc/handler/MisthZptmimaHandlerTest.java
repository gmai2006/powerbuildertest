...
import com.test.abc.dao.DefaultMisthZptmimaDao;
...
public class MisthZptmimaHandlerTest {
  ...
  private File createRecordInputStreamFromJsonFile(String inputFile, Charset charset) {
    try {
      ...
      String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
      final MisthZptmima[] records = gson.fromJson(json, MisthZptmima[].class);
      String header =
          createHeader(
  ...
  private String createCsvRecord(final MisthZptmima record) {
    return TestUtils.getObject(record.getId())
        ...
        + ","
        + TestUtils.getObject(record.getDesctmima());
  }
