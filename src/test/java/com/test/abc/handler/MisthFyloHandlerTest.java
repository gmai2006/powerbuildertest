...
import com.test.abc.dao.DefaultMisthFyloDao;
...
public class MisthFyloHandlerTest {
  ...
  private File createRecordInputStreamFromJsonFile(String inputFile, Charset charset) {
    try {
      ...
      String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
      final MisthFylo[] records = gson.fromJson(json, MisthFylo[].class);
      String header =
          createHeader(
  ...
  private String createCsvRecord(final MisthFylo record) {
    return TestUtils.getObject(record.getId())
        ...
        + ","
        + TestUtils.getObject(record.getDescfylo());
  }
