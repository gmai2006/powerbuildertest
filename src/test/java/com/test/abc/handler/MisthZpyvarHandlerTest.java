...
import com.test.abc.dao.DefaultMisthZpyvarDao;
...
public class MisthZpyvarHandlerTest {
  ...
  private File createRecordInputStreamFromJsonFile(String inputFile, Charset charset) {
    try {
      ...
      String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
      final MisthZpyvar[] records = gson.fromJson(json, MisthZpyvar[].class);
      String header =
          createHeader(
  ...
  private String createCsvRecord(final MisthZpyvar record) {
    return TestUtils.getObject(record.getId())
        ...
        + ","
        + TestUtils.getObject(record.getDescyvar())
        + ","
