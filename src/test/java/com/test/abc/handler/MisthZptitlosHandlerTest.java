...
import com.test.abc.dao.DefaultMisthZptitlosDao;
...
public class MisthZptitlosHandlerTest {
  ...
  private File createRecordInputStreamFromJsonFile(String inputFile, Charset charset) {
    try {
      ...
      String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
      final MisthZptitlos[] records = gson.fromJson(json, MisthZptitlos[].class);
      String header =
          createHeader(
  ...
  private String createCsvRecord(final MisthZptitlos record) {
    return TestUtils.getObject(record.getId())
        ...
        + ","
        + TestUtils.getObject(record.getDesctitlos());
  }
