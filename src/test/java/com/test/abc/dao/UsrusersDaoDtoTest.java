...
public class UsrusersDaoDtoTest {
  ...
  public static void beforeClass() {
    ...
    try {
      String json =
          FileUtils.readFileFromResource2String(usrusersInputFile, Charset.defaultCharset());
      usrusersRecords = gson.fromJson(json, Usrusers[].class);
      Arrays.stream(usrusersRecords).skip(1).forEach(o -> usrusersDao.create(o));
    } catch (IOException ex) {
      ex.printStackTrace();
    }
    try {
      String json =
          FileUtils.readFileFromResource2String(usrmembersInputFile, Charset.defaultCharset());
      usrmembersRecords = gson.fromJson(json, Usrmembers[].class);
      Arrays.stream(usrmembersRecords).skip(1).forEach(o -> usrmembersDao.create(o));
    } catch (IOException ex) {
      ex.printStackTrace();
    }
  }
